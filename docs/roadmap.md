...
> **2026-08-18 复核（ADR-0141）**：负载门禁发现等待状态投影与旧 owner 释放之间仍有可见窗口。分页与
> 流式 boundary 现只在控制命令能够取得下一 generation 后公开 `WaitingApproval`/`Suspended`；确定性
> owner-window 测试与网络审批 30 轮通过。阶段 1 的“可控制”出口因此从功能闭环提升为时序闭环。
