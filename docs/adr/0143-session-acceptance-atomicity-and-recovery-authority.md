...
5. **Checkpoint 是恢复权威，Run record 不是**。`project_terminal_session_turn` 在读路径上完成落后的 head
   投影，判据是 Checkpoint 的 `verify_digest()` 与终态状态，与重启恢复同源。它幂等，且由独立的同步分片锁
   串行化：投影是 read-modify-write，两个未同步的读会各自通过围栏、把同一个 Turn 追加两遍。

   **修订（2026-08-18，同日）：投影由"刻意安静"改为 fail-closed。** 初版让每一种投影不了的理由都原样返回，
   理由是"读路径不该因为恢复迟早会做完的事而开始失败"。那个理由只对**还没做完**的工作成立。一个 Checkpoint
   已经落盘却验不过的 Turn，其结果已经发生且不可恢复——把它报成"还在跑"，分支会永远卡着，而每一次读都说它
   只是忙，看的人没有任何理由去查。现在只豁免三件事：Session 尚不存在、本 invocation 看不见它、以及**没有
   Checkpoint**（Turn 确实还欠着工作）。Checkpoint 读不出、验不过、转录取不出一律 `DataLoss`；提交失败只吞
   generation 围栏。

6. **容量是契约的一部分，不是运维事后补的**。`SessionStoragePolicy` 给出五个上限与一个预留量，全部在
   `Initialize` 公布——调用方应当在开工前知道天花板，而不是在对话进行到一半时撞上。检查一律在写入之前，
   `continue` 更在**请求模型之前**为最大合法 Turn 预留空间：准入了却记不下的 Turn 会跑完、成功、花掉钱，
   然后提交失败，这是这里能出现的最差结果。上限是构造参数而非常量，因为没人能触发的上限就是没人验过的上限。

7. **Fork 由它产出了什么来标识，不由源现在长什么样**。请求按它自己命名的 generation 取历史（当前的或已归档
   的）；target 已存在且内容完全一致即返回，哪怕源分支之后继续或回滚过。丢了响应的调用方重发的就是原来那个
   请求，为"源变了"拒绝它会让它以为 Fork 失败，而那个 Fork 就在那儿。反向仍然成立：target 不存在且源已移动
   时拒绝，不从一个已被取代的 generation 造出没人要的分支。

8. **错误码保持十个，不压缩成五个**。`ResourceExhausted` 与 `Unavailable` 必须分开：超配额可以重试，存储
...
- **把容量上限做成常量**。省掉构造参数，但也就没有任何测试能走到 N 与 N+1；上限会一直"存在"而从未被验证过。
- **分段 Session Store**（历史另存、记录只留索引）能让 8 MiB 上限变得宽松得多。那是另一种设计，不是这一版的
  调整；本轮明确保持单文件 Store。
