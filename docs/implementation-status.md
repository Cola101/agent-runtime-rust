...
- 外部调用方认证**已有第一段**（ADR-0123）：`RuntimeInvocation` gRPC 要求运维形状 token 与
  `runtime.invoke` scope，身份取自 claims，真实网络闭环已跑通。但**部署出来的二进制仍不提供该面**
  （`main.rs` 只绑 Unix socket），也**没有接 TLS/mTLS**，因此在接线之前不得在回环之外暴露，
  更不得声称远端调用已完成。进程内调用仍必须选择预注册的完整 invocation Profile，且已有全局/tenant/
