...
| OpenClaw 可比协同语义（不含 Edge） | **约 55–60%** | durable 多租户身份、恢复和副作用围栏较强；已补有界冷 Event/history gap，但 SQLite Session 事务、迁移/quarantine、压缩归档、Gateway 运维和跨平台产品能力仍落后 |
...
request state/form Accept 并完成同一 Run。exact external gate **1 通过、0 失败**；无 fixture 环境时 dev package
生成退出 2 的 stub，普通 workspace 不会假绿。本阶段没有发现协议缺陷、没有修改生产协议代码，也没有启动
外部服务或使用凭据。这仍只是 N=1 的外部实现，Streamable HTTP、多第三方 MCP、真实 OAuth 和三类 Provider
矩阵未完成，因此总体仍为 70–75%。证据见
`docs/evidence/2026-08-17-codex-mcp-2026-cross-project-compatibility.md`。

同日有界冷 Event 历史完成（ADR-0136）：终态 retention 可在删除热 Run 前，流式生成内容寻址 JSONL
对象，完成 fsync、SHA-256 readback 和摘要索引提交；Event Cursor 与 subscription 在 tombstone、索引、对象、
连续事件链和终态全部一致时继续返回退休 Run 历史。单 Run、Workspace、tenant 的条数/字节上限共同生效，
多个 Workspace 不会乘法放大 tenant 预算；淘汰和超限只产生真实 `history_gap`，承诺对象损坏则 typed
`corrupt_log`。Unix 权限为目录 0700、index/object 0600，索引有 16 MiB 硬上限，读取/校验不整文件入内存。
该能力默认关闭：首次默认归档使 1000 Run 门禁约由 160 秒升至 195 秒，已撤回默认成本且未放宽门禁。
Runtime Host **231 通过、0 失败、1 个外部 fixture 忽略**；全包内 1000 Run 为 171.75 秒、1000 in-flight /
32 admitted 为 43.96 秒，Clippy、格式与差异门禁通过。尚无压缩、自动 quarantine、外部对象存储、硬件掉电、
共享文件系统和 Windows 证据，总体仍为 70–75%。证据见
`docs/evidence/2026-08-17-bounded-cold-event-history.md`。

同日统一终态 Run 收敛完成（ADR-0135）：普通 one-shot Run 现在与 Root Session/子代理一样，在 terminal
...
通过。证据见 ADR-0112 与
`docs/evidence/2026-08-15-graph-aware-retention-and-segmented-ledger.md`。当时仍未证明 1000 同时 active Run、
公开 history gap、冷归档读取、外部 tombstone 转储和非 Unix 跨进程单写；前两项后来由 ADR-0114/0136 完成。
...
| MCP 异步发现与单写协调 | 无 NATS 依赖地并行执行逐 attempt 网络发现；后台任务只返回不可变结果。Coordinator 仅接受 attempt ID，从 Worker 权威状态取得命令与取消令牌，串行完成目录挂载；新 Run 挂载后才 Start，恢复 Run 挂载后必须通过 Checkpoint 目录与发现策略校验。真实慢 Run 阻塞时快 Run 进入 ModelInvocation，真实恢复只在精确目录重建后返回可恢复。required Server 在冻结预算内持续 unavailable 时，Coordinator 保留 status 并由 Kernel 从 Queued/Running 提交唯一 `run.failed(kind=required_mcp_unavailable)`；不伪造 Start、不调用模型、不由 detached adapter 旁路写终态 | `ADR-0043`、`ADR-0044`、`ADR-0124`、`mcp_discovery_supervisor.rs`、`mcp_discovery_coordinator.rs`、`mcp_end_to_end.rs`、`embedded_multi_tenant.rs`、`docs/evidence/{2026-08-09-mcp-single-writer-coordinator,2026-08-17-required-mcp-startup-terminal-outcome}.md` |
...
| Runtime 网络调用契约 | `contracts/proto/runtime.proto` 的 `RuntimeInvocation`：Submit/Control/ReadEvents，由新 crate `agent-runtime-invocation-protocol` 编译（旧 `RuntimeControl` 是 0 实现、0 引用、无人编译的空契约，已被取代）。调用方必须持运维形状 token（schema 5）+ 独立 scope `runtime.invoke`；Run 形状携带该 scope 仍 `permission_denied`。tenant/application/workload identity 取自已验证 claims，请求体只能同意；workspace/agent_version/model_policy 选择 Profile，安全性来自 Profile 以完整六元组为键。生命周期边界在线路上保持 typed（`Terminal`/`Retired` 各带字段），`history_gap` 显式。协议中立文档走 `bytes *_json`（与 `model_gateway.proto` 一致）。状态标记取自 serde 而非 `Debug`。错误不透传 state root 路径；未注册 Profile 返回 `permission_denied` 而非 `not_found`。真实闭环已验证：调用方只持地址与 bearer token，Submit → typed 边界排空游标 → 读回模型回答，Provider 为真实回环 HTTP/SSE。`serve` 已接线且**默认关闭**：未设 bind 地址即无监听器，设了却缺 mTLS 材料或验签公钥则拒绝启动（由 spawn 真实二进制的测试证明），该检查排在所有配置之前。mTLS 已实测：`rcgen` 现场生成证书，真实 Run 穿过 mTLS 完成，且**无客户端证书**与**他方 CA 证书**均被拒（拒绝测试各带合法证书对照，防假绿）。控制路径成功闭环：网络取消停掉活着的 Run、同 id 重放返回同一收据、同 id 换 action 返回 `failed_precondition`。审批成功闭环，且参数**仅凭事件字节**发现，全程不读状态目录。跨进程恢复已证：第一个 Runtime 连同服务、执行任务与 state-root 锁整体消失后，调用方只带 `run_id` 重连、读到完整历史并完成审批，`run.started` 只出现一次。流式 `WatchEvents` 与分页共用排他游标，断流可续且不重送。确定性不可选路由与持久 Provider 重试预算耗尽已和可恢复调用错误分型：前两者经 Kernel 提交唯一 `run.failed`，事件游标返回 typed failed 终态，不再把业务失败误报为 `DataLoss`；仍有预算的 503 保留替代 Host 恢复。公开显式 Resume 成功路径后来由 ADR-0127 补齐；仍未做 Java SDK | `ADR-0123`、`ADR-0127`、`contracts/proto/runtime.proto`、`runtime/crates/runtime-invocation-protocol/`、`runtime/apps/runtime-host/src/grpc.rs`、`runtime/apps/runtime-host/tests/grpc_invocation_{identity,loop,mtls,control,approval,recovery,watch}.rs`、`runtime/apps/runtime-host/tests/{invocation_surface_config,embedded_multi_tenant}.rs`、`docs/evidence/{2026-08-17-runtime-network-invocation-surface,2026-08-17-public-explicit-resume}.md` |
| 控制命令接纳与 Kernel 终态权威 | 审批与 MCP 输入在 receipt、owner epoch、Run record 变化前同时校验 adapter 投影和摘要有效的 Worker Checkpoint；子代理目标必须属于 root Checkpoint 当前持有的 lineage。错误决定无 receipt、无状态变化；已接纳决定遇到 Host/事件存储错误时保留 `*Decided + Accepted receipt` 给替代 Host 恢复，不再合成 `Finished/failed`。`drive_recorded` 只从 Kernel terminal event 投影终态。等待审批/输入的取消也恢复 Checkpoint 并由 Kernel 提交唯一 `run.cancelled`，Event Cursor 得到 typed cancelled。公开 MCP 输入网络闭环由后续 ADR-0126 补齐 | `ADR-0125`、`runtime/apps/runtime-host/src/{embedded.rs,lib.rs}`、`runtime/apps/runtime-host/tests/{embedded_control,approval_flow,subagent_approval,execution_cancellation}.rs`、`docs/evidence/2026-08-17-control-command-terminal-authority.md` |
| 公开 MCP 输入回答与版本绑定 | `mcp.input.required` 公开独立 `input_version`，与 input id、binding 和 request set 一起供无状态调用方原样回送；协议、Worker 与 Embedded 共享唯一版本常量。无效版本经 typed `InvalidControlCommand` 映射为 gRPC `InvalidArgument`，不泄漏内部错误。真实网络调用方在第一 Runtime 整体消失后，只凭 token、Run ID 与事件恢复 form Accept；错误版本零 continuation，正确版本续传原 opaque state、同一 Tool 与唯一 Run 终态。仍未外推到真实外部 MCP、跨机器、URL 网络专项或 Java SDK | `ADR-0126`、`runtime/crates/{protocol,kernel}`、`runtime/apps/{worker,runtime-host}`、`runtime/apps/runtime-host/tests/grpc_invocation_mcp_input.rs`、`docs/evidence/2026-08-17-public-mcp-input-over-network.md` |
| 公开显式 Resume 与冻结模型预算 | 第一 Runtime 在模型请求已出网但未持久响应时整体消失；替代 Runtime 以原 Run、旧 owner epoch 和幂等 command id 经真实 gRPC 显式 Resume。默认单次同 Provider 预算正确收敛为 failed，不因 Resume 增额；冻结两次尝试时 epoch 单调提升、route journal 只做第二次调用、同一 Run 成功且 `run.started` 唯一。同 command id 终态重放返回相同 digest/收据，无第三次 Provider 调用。全 Profile 自动扫描由后续 ADR-0128 补齐；仍未外推到真实厂商、跨机器或分布式 command ledger | `ADR-0127`、`contracts/proto/runtime.proto`、`runtime/apps/runtime-host/tests/grpc_invocation_recovery.rs`、`docs/evidence/2026-08-17-public-explicit-resume.md` |
| 多 Profile 孤儿 Run 恢复 | `EmbeddedRuntime` 内部扫描全部 immutable Profile，坏租户 typed failure 不阻塞健康租户；Run 计划按 Profile round-robin 进入共享公平准入。单 Profile/全量扫描由同一恢复门串行，跳过本进程仍 active 的 Run；并发双扫描只接纳一次。Kernel 终态事件会收敛 Run record 与 Accepted receipt，避免事件已终态而命令重放仍非终态。两个租户真实 HTTP/SSE 请求在 Host replacement 后各只执行允许的第二次尝试并成功。生产宿主调用时机、跨机器 owner 选举、分布式 command ledger 和告警仍未实现 | `ADR-0128`、`runtime/apps/runtime-host/src/embedded.rs`、`runtime/apps/runtime-host/tests/embedded_recovery_all.rs`、`docs/evidence/2026-08-17-multi-profile-orphan-recovery.md` |
| JSONL 崩溃尾部修复 | Event Log 以最终换行为单行提交标记；回放、分页和流式订阅只忽略 EOF 唯一未换行片段，已换行坏 JSON/空行/超限行继续 fail-closed。下一次 append 反向找到最后提交边界、截断并同步后再续写；流式订阅保存完整行字节偏移，重开不从头重放。真实审批恢复和阻塞 Provider 的 live subscription 都跨半行完成唯一终态，sequence 连续。未证明主机掉电、共享文件系统或任意介质损坏 | `ADR-0129`、`runtime/apps/runtime-host/src/{lib.rs,embedded.rs}`、`runtime/apps/runtime-host/tests/{approval_flow,embedded_multi_tenant}.rs`、`docs/evidence/2026-08-17-jsonl-torn-tail-repair.md` |
| 权威目录扫描失败关闭 | Run 恢复枚举和 Session 归属扫描只有 `NotFound` 可解释为空；目录损坏、权限/I/O 和逐项读取错误全部 typed fail-closed。坏 Profile 在聚合恢复报告中隔离，不会伪报零 Run；Session 权威无法确认时不发布 Run 终态。52 项相关回归通过。自动 quarantine、修复工具和跨机器共享存储未实现 | `ADR-0130`、`runtime/apps/runtime-host/src/lib.rs`、`runtime/apps/runtime-host/tests/{embedded_recovery_all,embedded_multi_tenant}.rs`、`docs/evidence/2026-08-17-authoritative-directory-scan-errors.md` |
| 本地权威文件耐久替换 | Session、Run record、Checkpoint、Embedded control receipt、retention ledger/segment、子代理结果和 Tool reconciliation 共用 `write→sync file→rename→sync parent`；文件/目录同步错误 fail-closed。Event 保持追加提交标记。将高频模型路由 journal 逐次强同步会使 1000 Run 门禁超过 200 秒，已撤回且未放宽阈值；恢复后为 112–119 秒。模型路由 journal 当时仍缺追加 WAL，现已由 ADR-0132 完成；Provider 健康仍是可重建缓存，硬件掉电与 Windows 未验证 | `ADR-0131`、`runtime/apps/runtime-host/src/durable_file.rs`、`runtime/apps/runtime-host/src/{lib.rs,embedded.rs,retention.rs}`、`docs/evidence/2026-08-17-durable-file-replacement-boundary.md` |
...
| 有界冷 Event 历史 | opt-in 的内容寻址 JSONL 冷层在热删除前 fsync、SHA-256 readback 并提交摘要索引；Cursor/subscription 重验 tombstone、对象和完整事件链。单 Run/Workspace/tenant 双维 cap、跨 Workspace tenant 聚合、策略降低回收、真实 gap、损坏 fail-closed、0700/0600 权限和 16 MiB index hard limit 已验证。默认关闭；无压缩、quarantine、外部 store、共享文件系统和 Windows 证据 | `ADR-0136`、`runtime/apps/runtime-host/src/{event_archive.rs,embedded.rs,retention.rs}`、`runtime/apps/runtime-host/tests/embedded_retention.rs`、`docs/evidence/2026-08-17-bounded-cold-event-history.md` |
...
- 受控出网 HTTP 与 MCP 的 Tool 制品；Kubernetes Job/Kata 沙箱生命周期及 OCI 签名验证。
  **写文件与 Shell 已实现**（ADR-0036、0038），但都只在 macOS 上受容器约束，
  Linux 上没有 `landlock` 等价物。**这条限制现在由代码执行，不再只是本文的一句话**（ADR-0122）：
  非 macOS 上 `TrustedNative` 在 Workspace 解析与进程创建之前 typed fail-closed，
  而不是像此前那样静默降级为裸执行。这不是新增隔离能力，是让隔离的缺席变成显式拒绝。
...
- 外部调用方认证**已完成**（ADR-0123）：`RuntimeInvocation` gRPC 要求运维形状 token 与
  `runtime.invoke` scope，身份取自 claims；`serve` 已接线，默认关闭且无 mTLS 不可开启；
  提交、分页与流式事件订阅、审批、取消、跨进程崩溃恢复均已由独立网络调用方闭环验证。
  **但这全部是回环证据**：没有真实远端网络、没有跨机器、没有 Java SDK，因此**不得据此声称
  云端 Runtime 服务或 Java 集成已完成**。进程内调用仍必须选择预注册的完整 invocation Profile，且已有全局/tenant/
