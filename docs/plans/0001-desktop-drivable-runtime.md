...
### 1.6 同一类问题：`approval_flow` 在缺二进制时失败得不知所云

`approval_flow` 的 9 个测试在 `agent-trusted-workspace-tool` 未构建时**全部超时 60 秒**，消息是
"agent.close did not reap…"式的"审批没被记录"——指向审批链，而真正的原因是
`trusted_tool_binary()` 静默返回 `None`，于是根本没有工具可注册。

**每次完整 `cargo clean` 之后都会重演一次**（本批已经遇到两次，第二次正是上一批交付后清缓存造成的）。
构建该二进制后 9/9 通过、耗时 3.69 秒。

与 1.5 是同一类缺陷：**测试指名了一个它区分不出的原因**。修法同样是让失败说清，而不是放宽任何东西——
没有工具时应当立刻说"没有可用的 trusted tool 二进制"，而不是等 60 秒再说审批没来。
