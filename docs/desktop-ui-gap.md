...
| 20 | 模型容灾可视 | 部分接入 | Provider 失败与重试现在说得出**是哪一个、为什么、等多久、第几次**：八种 `ModelErrorKind` 各有自己的话（密钥不对要去改配置，限流是等一等，上下文溢出是换个说法），不认识的原样打印。**还没有**：route journal 本身的入口，以及「换到了另一个 Provider」这件事的可视 |
