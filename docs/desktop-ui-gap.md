...
| 6 | Diff 逐块接受 / 拒绝 | 部分接入 | 审批一次 `workspace.write_text` 就是审批一次覆写，而卡片此前只显示整段新内容，不显示它和文件现状的差别 —— 那个人被问的唯一问题得去另一个面上凭记忆回答。现在转录和待决定队列**两处**都画出逐行对比（同一个组件），文件读不到就说是新文件还是打不开，太大就明说不比。**还没有**：逐块接受/拒绝（我们没有结构化 diff，写入是整文件替换） |
