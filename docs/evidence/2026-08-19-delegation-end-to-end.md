# 子代理端到端：通了；失败的孩子回不来，也修好了（2026-08-19）
...
### 修好了：缺的成分是「同一批里还有一个工具调用」

前两次复现都没打到，因为我送的批是 `[内容, usage, finish_reason]`——终局落在批尾，
后面没有东西，什么都不会发生。**真实那次的提示词是「读一下 notes.txt」，含「读」，
stub 走 read 分支发的是工具调用**，所以那一批是：

```
[内容, tool_call, usage, finish_reason]
              ↑ 终局在这里发生，而它后面还有两条
```
...
```
Runtime(Execution("execution attempt is already terminal"))
```

**修法是把本来就存在的终局检查从 `for` 循环外面挪到里面**
（`lib.rs`，`for event in events` 的第一句）。attempt 一旦终局，这一批剩下的事件
不再适用：终局已经发出去了，而一个已经到达终局的 attempt，不会被 provider
在同一口气里说的下一句话改变。

### 这一条修复直接解掉了委托那半

在真实 host 上验证（干净 state root + stub provider + 掐到 1 token 的子代理预算）：

| | 修之前 | 修之后 |
| --- | --- | --- |
| 子 Run | `run.failed{budget_exhausted}` | 同上 |
| 父 Run | **停在 `suspended`，永远** | **`run.succeeded`** |
| 宿主 ERROR 行 | 1 | **0** |

父 Run 成功是对的：失败的子代理作为一个 `is_error` 的**工具结果**回到模型，
模型看到之后作答——这正是 `SubagentResultDelivery` 上那个字段一直准备着的事。
所以本文档前面写的「协议、内核、界面三层都准备好了，只有本地 host 把它变成
没人接的错误」是对的，只是**根因比我当时以为的浅**：不是投递缺失，
是一个 Run 在自己的终局之后多走了一步。
...
**（这一段是当时的判断，保留。）** 当时以为要先决定「子代理失败对父意味着什么」，
是一个契约决定而不是一处补丁。结果契约本来就是对的，要改的只有一处循环。
判断错的方向是**把问题想大了**，而看清它靠的是把复现做窄——一次只加一个成分，
直到那一批里出现工具调用。
