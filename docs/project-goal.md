...
当前采用**桌面优先、内核权威**的双轨主线：优先交付一个可日用的 macOS Apple Silicon 桌面 Alpha，
同时继续对标 Codex 的执行内核与 OpenClaw 的协同语义。桌面端不是演示壳，也不复制 Agent 状态机；
它必须通过稳定 owner/runtime 契约驱动同一个 Rust Runtime，并用真实桌面工作流暴露和验证内核缺口。

桌面 Alpha 复用现有 Electron + React 实现，不在此里程碑切换 Tauri。它至少完成安全的 Profile/credential
配置、Session 创建/恢复、流式输出、Tool 审批/拒绝/取消、历史、基础 Workspace 交互、明确错误状态，
以及应用退出后 Runtime 完全停止。最终验收必须在干净目录以可分发 artifact 完成真实闭环。

Runtime 不得依赖 Docker、虚拟机、Kubernetes、Java、PostgreSQL 或 NATS 才能启动和完成一次 Run。
当前不扩展 Edge、Java 控制面、云集群和 Windows/Linux 桌面；这些不是桌面 Alpha 的前置条件。GUI、
控制面、节点协议和部署编排可以驱动稳定公共契约，但不得掩盖内核缺口或迫使内核依赖某个上层实现。

## 固定实施顺序

1. **桌面最小闭环**：启动与恢复 Runtime、配置 Profile/credential、创建/继续 Session、流式事件、审批、
   取消、历史和基础 Workspace 交互。
2. **桌面安全与生命周期**：renderer 不接触凭证或 owner token；退出应用停止 Runtime，但不伪造用户取消；
   崩溃恢复、错误可见、单实例与进程树回收必须成立。
3. **可分发验收**：在干净目录安装和启动，完成真实 Provider、Tool 审批、中断恢复、升级迁移与退出，
   不依赖开发仓库、Java 或云服务。
4. **按桌面暴露的阻塞项补内核**：只优先修复真实工作流证明会阻塞可用性的 Agent Loop、模型、Tool/MCP、
   Session、Checkpoint、权限、预算与多租户缺口；不为 UI 复制状态。
5. **持续对标**：每批实现都记录 Codex/OpenClaw 已有语义、我们的差距与有意增强；对标不等于无差别复制，
   不影响桌面 Alpha 的能力留在后续内核路线图。

## 当前里程碑

ADR-0142 已完成 `RuntimeClient v1` 与能力协商，ADR-0143 已收口 Session 创建、继续、分支、回滚、查询
与恢复语义，ADR-0144 已把 owner 作用域和应用生命周期接到独立桌面进程可达的面上。当前里程碑不再等待
全部内核路线完成后才开发 GUI，而是让现有桌面客户端与 Profile/credential、可分发 artifact 两个硬门禁
并行收敛，以一个真实可日用的桌面闭环作为出口。
