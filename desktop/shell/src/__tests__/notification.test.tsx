...
import { App } from "../App";
import {
  APPROVAL_ID, installFakeRuntime, RUN_DONE, RUN_LIVE, RUN_UNJUDGED, RUN_WAITING,
} from "./fake-runtime";

/// The two things the fixture's runtime is blocked on, in the order the queue
...
describe("what the window tells the host is waiting", () => {
  it("reports the whole queue, not the first thing in it", async () => {
    ...
    expect(await firstReport(bridge)).toEqual([UNJUDGED, ASKED]);
  });
  ...
  it("names them the same things on every poll, so the host can tell they are the same things",
    async () => {
      const bridge = await mounted();
      // The store polls; the same two are read again and reported again.
      ...
      expect([...named]).toEqual([`run:${RUN_UNJUDGED} approval:${APPROVAL_ID}`]);
    });
});
