    ...
    pub async fn list_tools(
        ...
        if server.protocol_revision == McpProtocolRevision::V2026_07_28 {
            let discover = self
                ...
                .await?;
            validate_modern_discovery(&discover)?;
            let result = self
                .call_modern_json_rpc(
                    &http,
                    server,
            ...
            return catalog_from_list_result(&server.name, &result);
        }
        let session = self
        ...
        let result = self
            ...
            .await?
            .0;
        catalog_from_list_result(&server.name, &result)
    }
...
fn validate_modern_discovery(result: &serde_json::Value) -> Result<(), McpFederationError> {
    ...
    if !result["capabilities"]["tools"].is_object() {
        return Err(McpFederationError::Protocol(
            "MCP server did not advertise the tools capability".into(),
        ));
    }
    Ok(())
}
...
fn validate_initialize_result(result: &serde_json::Value) -> Result<(), McpFederationError> {
    ...
    if !result["capabilities"]["tools"].is_object() {
        return Err(McpFederationError::Protocol(
            "MCP server did not negotiate the tools capability".into(),
        ));
    }
    Ok(())
}
